...
function ShoppingCart() {
  ...
  function calculateTotalPrice() {
    ...
    for (let i = 0; i < productsInShoppingCart.length; i++) {
      totalPrice += productsInShoppingCart[i].price * productsInShoppingCart[i].quantity*84; 
    }
    return totalPrice;
  }
        ...
        <>
          {productsInShoppingCart.map((eachProduct, index) => (
            <div id="single-cart-container" key={index}>
              ...
              {/* <div id="edit">
                <div id="minus" onClick={() => dispatch(eachProduct.quantity<)} style={eachProduct.quantity < 2 ? otherStyle : defaultStyle}>-</div>
                <div id="quantity">{eachProduct.quantity}</div>
                <div id="plus" onClick={() => dispatch(add(eachProduct))}>+</div>
              </div> */}
              ...
              <div id="price">
                <span id="dolar-span">Rs.</span>
                <span id="price-span">{eachProduct.price * eachProduct.quantity*84}</span>
                <span
                  id="trash-icon"
